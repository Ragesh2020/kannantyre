...
$('#orderdetailsItems').on('keypress', '.Subcategory', function autocomplete(e) {
    ...
    $('.Subcategory').autocomplete({
        source: function (request, response) {
            $.ajax({
                url: 'http://etreesales.com/ProductsForSale/Getsubcat',
                dataType: "json",
                data: {
                    term: Subcategory
        ...
        //source: 'http://etreesales.com/ProductsForSale/Getsubcat',
        minLength: 1
    });
});
...
$('#submit1').click(function () {
    ...
    if (isAllValid) {
        ...
        $.ajax({
            type: 'POST',
            url: 'http://etreesales.com/ProductsForSale/Create',
            ...
            success: function (data) {
                if (data.status) {
                    ...
                    if ($('.chkrpt').is(":checked")) {
                        ...
                        window.open('http://etreesales.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchaseNo').val()).toString(), 'name', 'width=600,height=400');
                        a = "1";
                    }
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        if ($('#rptgnrt').is(":checked") == true) {
                            window.open('http://etreesales.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchno').val()).toString(), 'name', 'width=600,height=400');
                        }
                        window.open('http://etreesales.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchno').val()).toString(), 'name', 'width=600,height=400');
                        document.location.reload(true);
                    }
