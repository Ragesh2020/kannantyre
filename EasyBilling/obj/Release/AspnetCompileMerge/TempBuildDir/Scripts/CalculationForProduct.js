...
$('#orderdetailsItems').on('keypress', '.Subcategory', function autocomplete(e) {
    ...
    $('.Subcategory').autocomplete({
        source: function (request, response) {
            $.ajax({
                url: 'http://kannantyres.com/ProductsForSale/Getsubcat',
                dataType: "json",
                data: {
                    term: Subcategory
        ...
        //source: 'http://kannantyres.com/ProductsForSale/Getsubcat',
        minLength: 1
    });
});
...
$('#submit1').click(function () {
    ...
    if (isAllValid) {
        ...
        $.ajax({
            type: 'POST',
            url: 'http://kannantyres.com/ProductsForSale/Create',
            ...
            success: function (data) {
                if (data.status) {
                    ...
                    if ($('.chkrpt').is(":checked")) {
                        ...
                        window.open('http://kannantyres.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchaseNo').val()).toString(), 'name', 'width=600,height=400');
                        a = "1";
                    }
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        if ($('#rptgnrt').is(":checked") == true) {
                            window.open('http://kannantyres.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchno').val()).toString(), 'name', 'width=600,height=400');
                        }
                        window.open('http://kannantyres.com/api/Purchase/GetPurchaseListReport?prdctno=' + ($('.purchno').val()).toString(), 'name', 'width=600,height=400');
                        document.location.reload(true);
                    }
